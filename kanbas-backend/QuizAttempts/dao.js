...
export const startQuizAttempt = async (req, res) => {
  try {
    ...
    const previousAttempts = await QuizAttempt.find({ quizId, userId });
    ...
    if (quiz.multipleAttempts && previousAttempts.length >= quiz.attempts) {
...
export const submitQuizAttempt = async (req, res) => {
  try {
    ...
    const attempt = await QuizAttempt.findOne({ quizId, userId }).sort({ createdAt: -1 });
